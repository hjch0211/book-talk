...
export class VoiceChatManager {
    ...
    async handleParticipantJoined(participantId: string, participantName: string, shouldInitiateConnection: boolean = false): Promise<void> {
        ...
        // Always create peer connection when participant joins
        // This ensures we're ready to receive offers or send offers
        const peerConnection = this.createPeerConnection(participantId);
        this.peerConnections.set(participantId, peerConnection);

        // Always add local tracks to peer connection
        if (this.localStream) {
        ...
        // Only send offer if we should initiate
    ...
    async handleOffer(fromId: string, offer: RTCSessionDescriptionInit): Promise<void> {
        console.log(`📥 [${this.myAccountId}] handleOffer from ${fromId}`);

        if (fromId === this.myAccountId) {
            console.log(`⚠️ [${this.myAccountId}] Ignoring offer from self`);
            return;
        }

        // If participant not in our list yet, add them (discovered via offer)
        if (!this.participants.has(fromId)) {
            console.log(`🆕 [${this.myAccountId}] Adding participant ${fromId} discovered via VOICE_OFFER`);
            const participant: VoiceChatParticipant = {
                accountId: fromId,
        ...
        console.log(`🔍 [${this.myAccountId}] Checking peer connection for ${fromId}`);
        let peerConnection = this.peerConnections.get(fromId);
        if (!peerConnection) {
            console.log(`🔗 [${this.myAccountId}] Creating NEW peer connection for ${fromId}`);
            peerConnection = this.createPeerConnection(fromId);
            this.peerConnections.set(fromId, peerConnection);

            // Add local tracks only if peer connection was just created
            if (this.localStream) {
                console.log(`🎤 [${this.myAccountId}] Adding ${this.localStream.getTracks().length} local tracks`);
                this.localStream.getTracks().forEach(track => {
                    console.log(`  ➕ Adding track: ${track.kind}, enabled=${track.enabled}, readyState=${track.readyState}`);
                    peerConnection!.addTrack(track, this.localStream!);
                });
            } else {
                console.error(`❌ [${this.myAccountId}] No localStream available!`);
            }
        } else {
            console.log(`✅ [${this.myAccountId}] Using existing peer connection for ${fromId}`);
        }

        try {
            console.log(`🔧 [${this.myAccountId}] Setting remote description...`);
            await peerConnection.setRemoteDescription(offer);
            console.log(`✅ [${this.myAccountId}] Remote description set`);

            console.log(`🔧 [${this.myAccountId}] Creating answer...`);
            const answer = await peerConnection.createAnswer();
            console.log(`✅ [${this.myAccountId}] Answer created`);

            console.log(`🔧 [${this.myAccountId}] Setting local description...`);
            await peerConnection.setLocalDescription(answer);
            console.log(`✅ [${this.myAccountId}] Local description set`);

            console.log(`📤 [${this.myAccountId}] Sending VOICE_ANSWER to ${fromId}`);
            this.onSignalingMessage({
                type: 'VOICE_ANSWER',
                provider: 'CLIENT',
                debateId: this.debateId,
            ...
            console.log(`✅ [${this.myAccountId}] VOICE_ANSWER sent successfully`);
        } catch (error) {
            console.error(`❌ [${this.myAccountId}] Error in handleOffer:`, error);
            throw error;
        }
    }
