...
class DebateScheduler(
    ...
    /**
     * 매일 0시 실행
     * - 생성된 지 하루가 지났으며 host 외 참여자가 없는 토론 자동 종료 및 host 안내 메일 발송
     * - 생성된 지 1주일이 지난 토론 자동 종료
     */
        ...
        try {
            ...
            if (emptyCandidates.isNotEmpty()) {
                emptyCandidates.forEach { debate ->
                    debate.closedAt = now
                    debateRoundRepository.findByDebateIdAndEndedAtIsNull(debate.id!!)?.let { it.endedAt = now }
                        ...
                        try {
                            mailClient.send(
                                SendMailRequest(
                                    ...
                                    subject = "[BookTalk] 참여자 없이 토론이 종료되었습니다",
                                    html = buildEmptyDebateClosedHtml(debate.topic),
                                )
                            )
                        } catch (e: Exception) {
                            logger.error(e) { "빈 토론 종료 안내 메일 발송 실패: debateId=${debate.id}, to=${debate.host.email}" }
                        }
                    }
                }
            }
    ...
    private fun buildEmptyDebateClosedHtml(topic: String): String {
        return """<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>참여자 없이 토론이 종료되었습니다</title></head><body style="margin:0;padding:0;background-color:#f5f5f5;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="padding:24px 0;"><tr><td align="center"><table role="presentation" width="480" cellspacing="0" cellpadding="0" border="0" style="max-width:480px;background-color:#ffffff;border-radius:8px;box-shadow:0 6px 18px rgba(0,0,0,.06);padding:32px 28px 28px;"><tr><td align="center" style="padding:0 0 16px 0;"><h1 style="margin:0;font-size:20px;font-weight:700;color:#111827;">참여자 없이 토론이 종료되었습니다</h1></td></tr><tr><td align="center" style="padding:0 0 24px 0;"><p style="margin:0;font-size:14px;line-height:1.6;color:#4b5563;">토론 시작 후 다른 참여자가 입장하지 않아 토론이 자동으로 종료되었습니다.<br>불편을 드려 진심으로 죄송합니다.<br><br>다음에 토론을 개설하실 때는 다른 참여자들이 충분히 참여할 수 있도록<br><strong>토론 예정 시간을 넉넉하게 설정</strong>해 주시면 더 원활한 토론이 가능합니다.</p></td></tr><tr><td style="padding:0 0 16px 0;"><div style="background-color:#f9fafb;border-radius:8px;border:1px solid #e5e7eb;padding:20px 24px;"><p style="margin:0 0 8px 0;font-size:13px;color:#9ca3af;font-weight:500;">종료된 토론 주제</p><p style="margin:0;font-size:16px;font-weight:600;color:#111827;">$topic</p></div></td></tr><tr><td align="center"><p style="margin:0;font-size:12px;color:#9ca3af;">본인이 개설하지 않은 토론이라면, 이 메일을 무시하셔도 됩니다.</p></td></tr></table></td></tr></table></body></html>"""
    }
}