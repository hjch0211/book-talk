...
import { CardSlot, FloatWrapper, RotatingGroup, Scene, Wrapper, WrapperColumn } from './style';
...
interface Props<T extends Carousel3DItem> {
  ...
  renderNavigation?: (onPrev: () => void, onNext: () => void) => React.ReactNode;
  radius?: number;
  cardWidth?: number;
  cardHeight?: number;
  sceneHeight?: number;
...
export function Carousel3D<T extends Carousel3DItem>({
  ...
  const onPrev = () => selectByIndex(currentIndexRef.current - 1);
  const onNext = () => selectByIndex(currentIndexRef.current + 1);
  ...
  if (renderNavigation) {
    return (
      <WrapperColumn role="group" aria-label={ariaLabel} onKeyDown={handleSceneKeyDown}>
        {scene}
